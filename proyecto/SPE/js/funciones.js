...
function LlenadoSelect() {
    $.post('funciones/acceso.php', {
        ...
        if (respuesta == 1) { //No hay datos
            $('#columna1').html('<div class="alert alert-danger text-center" role="alert"> Estimado(a) : No hay solicitudes de permiso especial para mostrar</div>');
            $('#columna2').html('<div class="alert alert-danger text-center" role="alert"> Estimado(a) : No hay solicitudes de permiso especial para mostrar</div>');
...
function LlenarDatos(searchField, numeropaginas) {
    ...
    $.post('funciones/acceso.php', {
        ...
        /*Si hay datos en las solicitudes y además los roles son 12 o 13 
(Encargado(a) de personal o Jefe Sistema de Salud se remueve el hidden, 
esto es para que se vea el div o tabla de datos y los informes de permisos aprobados) */
        if (rol == 12 || rol == 13) { //Uso el hidden en vez de hide() de jquery porque el html carga más rápido y no se nota el cambio
        ...
        if (respuesta == 1) { //No hay datos
            ...
            $.each(res, function (i, val) {
                ...
                if (i < numeropaginas) {
                    if ((val.MOTIVO.search(regex) != -1) || (val.NOMBRE_FUNCIONARIO_SOLICITANTE.search(regex) != -1) || (val.RUT.search(regex) != -1)) {
                    ...
                    <li class="list-group-item ` + marcador + `">
                        <div class="media">
                            <div class="pull-left">
                                <div class="controls">
                                    <div class="btn-group">
                                        <button class="btn btn-success btn-sm" onclick="aprobar('` + val.ID_SPE + ',' + val.ID_PE + `')" title="Aprobar solicitud"><i class="fas fa-check" style="width: 15px;"></i></button>
                                        <button class="btn btn-danger btn-sm" onclick="rechazar('` + val.ID_SPE + ',' + val.ID_PE + `')" title="Rechazar solicitud"><i class="fas fa-times" style="width: 15px;"></i></button>
                                    </div>
                                </div>
                            </div>
                            <div class="media-body">
                                <div class="media-heading">
                                        <a href="javascript:void(0);" class="m-r-10">` + val.NOMBRE_FUNCIONARIO_SOLICITANTE + `</a>
                                ...
                                <span><small>Inicio:  ` + val.HORA_INICIO + `</small></span>  <span class="pl-1"><small>Término:  ` + val.HORA_TERMINO + ` </small> <span class="pl-1"><small>Dia Permiso:  ` + porciones_fecha_permiso[2] + ` de ` + mesesabreviados[porciones_fecha_permiso[1]] + `, ` + porciones_fecha_permiso[0] + ` </small> </span> <p class="msg">   <small>Motivo: ` + val.MOTIVO + `</small>  </p>
                            </div>
                        </div>  
                    </li>`;
                    }
