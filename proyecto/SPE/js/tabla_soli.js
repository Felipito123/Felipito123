$(document).ready(function () {
    tablapermisoespecial = $('#tabla_certificado').DataTable({
        "responsive": true,
        ...
        "oLanguage": {
            ...
            "sEmptyTable": "No hay solicitudes de permiso especial aprobados",
    ...
    $(document).on('click', '.btnVerPermiso', function () { //AQUI VEO LA BOLETA DE UNA SOLICITUD EN PARTICULAR
        let IDPE = (tablapermisoespecial.row($(this).closest('tr')).data().ID_PE);
        let RUTSOLICITANTE = (tablapermisoespecial.row($(this).closest('tr')).data().RUT);


        let comprobarDirectorio = new Request('CERTIFICADO_SOLICITUDES/' + RUTSOLICITANTE + '-' + IDPE + '/SPE.pdf');
        ...
        function mostrarcertficado() {
            Swal.fire({
                title: '<h2><strong>V E R  &nbsp; S O L I C I T U D&nbsp;  D E&nbsp;  P E R M I S O &nbsp;   E S P E C I A L </strong></h2>',
                html: `<div class="container-fluid">
                    <div class="row justify-content-center" id="framecertificado">
                        <iframe id="frameboleta" src="CERTIFICADO_SOLICITUDES/`+ RUTSOLICITANTE + `-` + IDPE + `/SPE.pdf" frameborder="0" scrolling="si" width="100%" height="750px"></iframe>
                    </div>
                </div> `,
                focusConfirm: false,
