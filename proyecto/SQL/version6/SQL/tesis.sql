...
CREATE TABLE IF NOT EXISTS `saludlosalamos2`.`tipo_remuneracion` (
  `id_tiporem` INT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `saludlosalamos2`.`tipo_dia` (
  `id_tipo_dia` INT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `saludlosalamos2`.`permisos_acumulados` (
  `id_permisos_acumulados` INT NOT NULL AUTO_INCREMENT,
  `id_spa` INT NOT NULL,
  `con_goce_remuneraciones` INT NOT NULL DEFAULT 0,
  `sin_goce_remuneraciones` INT NOT NULL DEFAULT 0,
  `otros` VARCHAR(150) NULL,
  PRIMARY KEY (`id_permisos_acumulados`),
  INDEX `fk_permisos_acumulados_solicitud_permiso_administrativo1_idx` (`id_spa` ASC),
  CONSTRAINT `fk_permisos_acumulados_solicitud_permiso_administrativo1`
    FOREIGN KEY (`id_spa`)
    REFERENCES `saludlosalamos2`.`solicitud_permiso_administrativo` (`id_spa`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
