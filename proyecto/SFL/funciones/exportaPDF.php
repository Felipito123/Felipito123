...
function ExportaPDFJefesDirecto(
    ...
    $exp1->MultiCell(145, 10, "F E R I A D O  L E G A L", 0, 'C', 0); //ancho,altura
...
function ExportaPDFEncPersonal(
    ...
    $porcionDesde = explode("-", $desde);
    $diaDesde = $porcionDesde[2];
    $mesDesde = $porcionDesde[1];
    $anoDesde = $porcionDesde[0];
    $nombredelmesDesde = strftime('%B', mktime(0, 0, 0, number_format($mesDesde)));

    $porcionHasta = explode("-", $hasta);
    $diaHasta = $porcionHasta[2];
    $mesHasta = $porcionHasta[1];
    $anoHasta = $porcionHasta[0];
    $nombredelmesHasta = strftime('%B', mktime(0, 0, 0, number_format($mesHasta)));
    /*=============================================PARA FORMATEAR LA FECHA ======================================*/
    ...
    $pdf->MultiCell(150, 5, utf8_decode("FOLIO: ") . codificarfolio($cortarrut), 0, 'R', 0);
    ...
    $pdf->MultiCell(145, 10, "F E R I A D O  L E G A L", 0, 'C', 0); //ancho,altura
    ...
    $pdf->MultiCell(150, 7, utf8_decode("Certifico que el(la) Sr(a) Funcionario(a) ") . utf8_decode($nombreusuario) .
        utf8_decode(", con cédula nacional de identidad N° ") . RutConCodVerificador($rutusuario) .
        utf8_decode(", perteneciente a ") . utf8_decode($SectoroUnidad) .
        utf8_decode(". Solicitó el permiso de feriado legal, ") .
        utf8_decode("para el dia ") . $diainicio . utf8_decode(" de ") . $nombredelmesinicio . utf8_decode(" del año ") . $anoinicio .  ".");
    ...
    $pdf->MultiCell(150, 7, utf8_decode("FERIADO ACUMULADOS DEL AÑO: " . $ano_feriado_acumulado_pfl));
    $pdf->MultiCell(150, 7, utf8_decode("DÍAS: " . $dias_feriado_acumulado_pfl));


    $pdf->SetFont('Courier', 'B', 15);
    $pdf->MultiCell(150, 7, utf8_decode("\nOBSERVACIÓN DE PERSONAL "));
    $pdf->SetFont('Courier', '', 12);
    $pdf->MultiCell(150, 7, utf8_decode("TIPO: " . $tipo));
    $pdf->MultiCell(150, 7, utf8_decode("DESDE: " . $diaDesde . utf8_decode(" de ") . $nombredelmesDesde . utf8_decode(" del ") . $anoDesde));
    $pdf->MultiCell(150, 7, utf8_decode("HASTA: " . $diaHasta . utf8_decode(" de ") . $nombredelmesHasta . utf8_decode(" del ") . $anoHasta));
...
function obtienedatos(
    ...
    $pdf->Image('../../perfil/firmas/' . $rutJefeSistemaSalud . '/' . $imagenfirmaJefeSistemaSalud, 85, 246, 30, 0, ''); //width,heigth,size
    $pdf->Ln(6);
    $pdf->Cell(38); //padding left
    $pdf->Cell(140, 3, '____________________________', 0, 0, 'L');
    $pdf->Ln(4);
