$(document).ready(function () {
    tablaferiadolegal = $('#tabla_certificado').DataTable({
        "responsive": true,
        ...
        "oLanguage": {
            ...
            "sEmptyTable": "No hay solicitudes de feriado legal aprobados",
    ...
    $(document).on('click', '.btnVerPermiso', function () { //AQUI VEO LA BOLETA DE UNA SOLICITUD EN PARTICULAR
        let IDPFL = (tablaferiadolegal.row($(this).closest('tr')).data().ID_PFL);
        let RUTSOLICITANTE = (tablaferiadolegal.row($(this).closest('tr')).data().RUTSOLICITANTE);

        let comprobarDirectorio = new Request('CERTIFICADO_SOLICITUDES/' + RUTSOLICITANTE + '-' + IDPFL + '/SFL.pdf');

        // console.log("IDPFL: "+IDPFL+" RUT: "+RUTSOLICITANTE);
        ...
        function mostrarcertficado() {
            Swal.fire({
                title: '<h2><strong>V E R  &nbsp; S O L I C I T U D&nbsp;  D E&nbsp;  P E R M I S O &nbsp;  \n F E R I A D O &nbsp;L E G A L </strong></h2>',
                html: `<div class="container-fluid">
                    <div class="row justify-content-center" id="framecertificado">
                        <iframe id="frameboleta" src="CERTIFICADO_SOLICITUDES/`+ RUTSOLICITANTE + `-` + IDPFL + `/SFL.pdf" frameborder="0" scrolling="si" width="100%" height="750px"></iframe>
                    </div>
                </div> `,
                focusConfirm: false,
