function pyramidBuilder(data, target, options) {
    ...
    var maxValue = Math.ceil(Math.max(
        d3.max(data, function(d) {
            return percentage(d.hombre);
        }),
        d3.max(data, function(d) {
            return percentage(d.mujer);
        })
    )/0.05)*0.05;
    ...
    leftBarGroup.selectAll('.bar.left')
        ...
        .attr('x', 0)
        .attr('y', function(d) {
            return yScale(d.edad) + margin.middle / 4;
        })
        .attr('width', function(d) {
            return xScale(percentage(d.hombre));
        })
        ...
        .on("mouseover", function(d) {
            tooltipDiv.transition()
                .duration(200)
                .style("opacity", 0.9);
            tooltipDiv.html("<strong>Edad entre " + d.edad + " años</strong>" +
                    "<br />   " + prettyFormat(d.hombre) +" son hombres"+
                    "<br />")
                    // "<br />" + (Math.round(percentage(d.hombre) * 1000) / 10) + "% del total")
    ...
    rightBarGroup.selectAll('.bar.right')
        ...
        .attr('width', function(d) {
            return xScale(percentage(d.mujer));
        })
        .attr('height', (yScale.range()[0] / data.length) - margin.middle / 2)
        .on("mouseover", function(d) {
            tooltipDiv.transition()
                ...
                .style("opacity", 0.9);
            tooltipDiv.html("<strong> Edad entre " + d.edad + " años</strong>" +
                    "<br />   " + prettyFormat(d.mujer) + " son mujeres"+
                    "<br />")
                    // "<br />" + (Math.round(percentage(d.mujer) * 1000) / 10) + "% of Total")
                .style("left", (d3.event.pageX) + "px")
                .style("top", (d3.event.pageY - 28) + "px");
        })
        .on("mouseout", function(d) {
