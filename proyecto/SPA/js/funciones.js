...
function LlenadoSelect() {
    $.post('funciones/acceso.php', {
        ...
        if (respuesta == 1 || respuesta == 2) { //No hay datos
            $('#columna1').html('<div class="alert alert-danger text-center" role="alert"> Estimado(a) : No hay solicitudes de permiso administrativo para mostrar</div>');
            $('#columna2').html('<div class="alert alert-danger text-center" role="alert"> Estimado(a) : No hay solicitudes de permiso administrativo para mostrar</div>');
...
esto es para que se vea el div o tabla de datos) */
        ...
        if (respuesta == 1 || respuesta == 2) { //No hay datos
            ...
            $.each(res, function (i, val) {
                ...
                if (i < numeropaginas) {
                    if ((val.MOTIVO.search(regex) != -1) || (val.NOMBRE_SOLICITANTE.search(regex) != -1) || (val.RUT_SOLICITANTE.search(regex) != -1) || (val.NOMBRE_REEMPLAZANTE.search(regex) != -1)) {
                    ...
                    <li class="list-group-item ` + marcador + `">
                        <div class="media">
                            <div class="pull-left">
                                <div class="controls">
                                    <div class="btn-group">
                                        <button class="btn btn_verdeOscuro btn-sm" onclick="aprobar('` + val.ID_SPA + ',' + val.ID_PA + `')" title="Aprobar solicitud"><i class="fas fa-check" style="width: 15px;"></i></button>
                                        <button class="btn btn-danger btn-sm" onclick="rechazar('` + val.ID_SPA + ',' + val.ID_PA + `')" title="Rechazar solicitud"><i class="fas fa-times" style="width: 15px;"></i></button>
                                    </div>
                                </div>
                            </div>
                            <div class="media-body">
                                ...
                                <small>Dia Permiso: ` + porciones_fecha_permiso[2] + ` de ` + mesesabreviados[porciones_fecha_permiso[1]] + `, ` + porciones_fecha_permiso[0] + ` | Tipo: ` + val.TIPO_DIA + `</small> <br>
                                <small> Reemplazo:  ` + val.NOMBRE_REEMPLAZANTE + ` </small> <br>
                                <small>Motivo: ` + val.MOTIVO + `</small>
                            </div>
                        ...
                        // output += `<hr >`;
                        // <span><small> Tipo: ` + val.TIPO_DIA + ` </small></span> <br>
                        // <small> Reemplazo:  ` + val.NOMBRE_REEMPLAZANTE + ` </small> <br>
